...
function splitIdPet(num) {
  ...
  const firstPart = numStr.slice(0, 3); // Dapatkan 3 karakter pertama
  const secondPart = numStr.slice(3); // Dapatkan sisanya

  return [parseInt(firstPart), parseInt(secondPart)];
}

// Berfungsi untuk mengganti variabel lingkungan
...
async function updateEnv(variable, value) {
  ...
  fs.readFile(envFilePath, "utf8", (err, data) => {
    ...
    // Membuat atau memperbarui variabel dalam file
    const regex = new RegExp(`^${variable}=.*`, "m");
    const newData = data.replace(regex, `${variable}=${value}`);

    // Periksa apakah variabel tidak ada dalam file, tambahkan di akhir
    if (!regex.test(data)) {
      newData += `\n${variable}=${value}`;
    }
