...
class ZooAPIClient {
  ...
  async checkProxyIP(proxy) {
    try {
      ...
      if (response.status === 200) {
        ...
        throw new Error(`Tidak dapat memeriksa IP proksi. Kode status: ${response.status}`);
      }
    } catch (error) {
  ...
  async AnswerDaily(hash, accountIndex, questKey, checkData) {
    ...
    try {
      ...
      if (response.status === 200 && response.data.success) {
        ...
        this.log(`Periksa misinya "${questKey}" kegagalan: ${response.data.error}`, "warning");
        return { success: false, error: response.data.error };
      }
    } catch (error) {
      this.log(`Kesalahan saat memeriksa misi "${questKey}": ${error.message}`, "error");
      return { success: false, error: error.message };
    }
  }
  ...
  async claimQuest(hash, accountIndex, questKey, checkData = null) {
    ...
    try {
      ...
      if (response.status === 200 && response.data.success) {
        this.log(`Klaim tugas "${questKey}" berhasil, terima hadiah.`, "success");
        return { success: true, data: response.data };
      } else {
        return { success: false, error: response.data.error };
      }
    } catch (error) {
      this.log(`Kesalahan saat mengklaim misi "${questKey}": ${error.message}`, "error");
      return { success: false, error: error.message };
    }
  }
    ...
    try {
      ...
      for (const quest of quests) {
        ...
        if (quest.checkType === "checkCode") {
          this.log(`Mulailah mengerjakan tugas menjawab pertanyaan harian...`, "custom");
        ...
        if (claimResult.success === true) {
          this.log(`Selesaikan misinya ${quest.key} | "${quest.title}", menerima ${quest.reward} menghadiahkan.`, "success");
        } else if (claimResult.error === "already rewarded") {
          this.log(`Layanan misi ${quest.key} "${quest.title}" telah selesai sebelumnya.`, "warning");
        } else {
          this.log(`Tugas tidak dapat diselesaikan atau perlu dilakukan secara manual ${quest.key} | "${quest.title}": ${claimResult.error}`, "warning");
        }
        await new Promise((resolve) => setTimeout(resolve, 1000));
      }
    } catch (error) {
      this.log(`Terjadi kesalahan saat mendapatkan daftar tugas: ${error.message}`, "error");
    }
  }
        ...
        for (const dbAnimal of dbData.dbAnimals) {
          if (!existingKeys.has(dbAnimal.key)) {
            ...
            if (hero.coins >= level1Price) {
              ...
              if (response.status === 200 && response.data.success) {
                this.log(`Beli dengan sukses ${dbAnimal.title}`, "success");
          ...
          if (dbAnimal) {
            ...
            if (nextLevelData && hero.coins >= nextLevelData.price) {
              ...
              try {
                ...
                if (response.status === 200 && response.data.success) {
                  this.log(`Meningkatkan ${dbAnimal.title} Naik level dengan sukses ${nextLevel}`, "success");
                }
              } catch (error) {
                if (error.response?.status === 500) {
                  this.log(`Tidak dapat meningkatkan ${dbAnimal.title}: ${error.message}`, "error");
                }
              }
            }
          }
  ...
  async countdown(seconds) {
    ...
    for (let i = seconds; i > 0; i--) {
      ...
      process.stdout.write(`[${currentTime}] [*] Tunggu ${remainingMinutes} phút ${remainingSeconds} detik untuk melanjutkan...`);
      await new Promise((resolve) => setTimeout(resolve, 1000));
    }
    readline.cursorTo(process.stdout, 0);
  ...
  async runAccount() {
    ...
    console.log(`=========Akun ${i + 1}| ${firstName + " " + lastName} | ${this.proxyIP} | Mulai nanti ${timesleep} Kedua...`.green);
    this.set_headers();
...
async function main() {
  ...
  console.log("Remake By Super BianzZz https://t.me/superbianz".yellow);
  ...
  while (true) {
        ...
        workerPromises.push(
          new Promise((resolve) => {
            ...
            worker.on("exit", (code) => {
              worker.terminate();
              if (code !== 0) {
                errors.push(`Kesalahan pekerja untuk akun ${currentIndex} keluar dengan kode: ${code}`);
              }
              resolve();
            });
          })
    ...
    await sleep(3);
    console.log("Remake By Super BianzZz https://t.me/superbianz".yellow);
    console.log(`=============Lengkapi semua akun | Tunggu ${settings.TIME_SLEEP} menit=============`.magenta);
...
if (isMainThread) {
  main().catch((error) => {
    console.log("Itu sebuah kesalahan:", error);
    process.exit(1);
  });
} else {
